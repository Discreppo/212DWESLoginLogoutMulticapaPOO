...
if (isset($_REQUEST['confirmarCambios'])) { 
    ...
    if (!UsuarioPDO::validarUsuario($_SESSION['user212DWESLoginLogoutMulticapaPOO']->get_codUsuario(), $_REQUEST['contraseñaActual'])) {
...
if ($entradaOK) { // Si el usuario ha rellenado el formulario correctamente rellenamos el array aFormulario con las respuestas introducidas por el usuario
     ...
     */ 
    $_SESSION['user212DWESLoginLogoutMulticapaPOO'] = UsuarioPDO::cambiarPassword($_SESSION['user212DWESLoginLogoutMulticapaPOO'], $_REQUEST['nuevaContraseña']);
    $_SESSION['paginaEnCurso'] = 'miCuenta'; // Asigno a la página en curso la pagina de miCuenta
    header('Location: indexLoginLogoutMulticapaPOO.php'); // Redirecciono al index de la APP
    exit;
